...
function allTaskPluginEmojis(symbols: DefaultTaskSerializerSymbols) {
    const allEmojis: string[] = [];

    Object.values(symbols.prioritySymbols).forEach((value) => {
        if (value.length > 0) {
...
export function makeDefaultSuggestionBuilder(
    ...
    return (
        ...
        if (includeDependencySuggestions(canSaveEdits)) {
            ...
            suggestions = suggestions.concat(addDependsOnSuggestions(symbols, allTasks, parameters, taskToSuggestFor));
        }
...
function addDependsOnSuggestions(
    ...
    const results: SuggestInfo[] = [];
    const dependsOnSymbol = symbols.dependsOnSymbol;
