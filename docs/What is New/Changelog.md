...
- X.Y.Z:
  ...
  - `hide task count` was added to the `hide_query_elements` preset, so that the `hide_everything` preset (which composes `hide_query_elements`) now also hides the task count. See issue #3899.
- 8.3.0:
