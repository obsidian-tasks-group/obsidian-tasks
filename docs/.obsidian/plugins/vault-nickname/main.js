...
var VaultNicknamePlugin = class extends import_obsidian.Plugin {
    ...
    ///
    onLayoutReady() {
        const originalDesktopVaultSwitcherElement = window.activeDocument.querySelector(
        ...
        if (!originalDesktopVaultSwitcherElement) {
            ...
            this.desktopVaultSwitcherElement = originalDesktopVaultSwitcherElement.createDiv(
                '.workspace-drawer-vault-switcher-vault-nickname-interceptor',
            );
            Object.assign(this.desktopVaultSwitcherElement.style, {
                position: 'absolute',
                top: '0',
                left: '0',
                width: '100%',
            ...
            this.desktopVaultSwitcherElement.addEventListener(
                'contextmenu',
                this.onVaultSwitcherContextMenu.bind(this),
            );
    ...
    onVaultSwitcherClicked(event2) {
        ...
        for (let vaultKey in vaults) {
            ...
            const vaultPluginSettingsFilePath = (0, import_obsidian.normalizePath)(
                [vault.path, VAULT_LOCAL_SHARED_SETTINGS_FILE_PATH].join(PATH_SEPARATOR),
            );
            ...
            menu.addItem((item) =>
                item
                    ...
                    .onClick(() => window.open(`obsidian://open?vault=${vaultKey}`)),
            );
        }
        menu.addSeparator();
        menu.addItem((item) =>
            item
                ...
                .setIcon('open-vault')
                .onClick(() => this.app.commands.executeCommandById('app:open-vault')),
        );
    ...
    async onVaultSwitcherContextMenu() {
        ...
        const menu = new import_obsidian.Menu();
        const showInFolderText = import_obsidian.Platform.isMacOS
    ...
    setAppTitle(vaultDisplayName) {
        ...
        if (this.settings.overrideAppTitle === 'override-app-title:vault-first') {
            window.activeDocument.title = [vaultDisplayName, documentTitle, obsidianVersion].join(titleSeparator);
        } else {
            window.activeDocument.title = [documentTitle, vaultDisplayName, obsidianVersion].join(titleSeparator);
        }
    }
    ...
    getVaultParentFolderName() {
        ...
        if (
            indexOfParentFolder < 0 ||
            !explodedVaultPath[indexOfParentFolder] ||
            !explodedVaultPath[indexOfParentFolder].trim()
        ) {
            return '';
        }
    ...
    getSharedSettingsFilePath() {
        return [this.app.vault.adapter.getBasePath(), VAULT_LOCAL_SHARED_SETTINGS_FILE_PATH].join(PATH_SEPARATOR);
    }
...
var VaultNicknameSettingTab = class extends import_obsidian.PluginSettingTab {
    ...
    display() {
        ...
        new import_obsidian.Setting(containerEl)
            ...
            .setTooltip(
                import_obsidian.Platform.isDesktop
