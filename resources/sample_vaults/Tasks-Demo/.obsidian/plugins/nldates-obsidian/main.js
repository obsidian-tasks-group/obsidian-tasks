...
function getYearlyNoteSettings() {
    ...
    try {
        const settings =
            (shouldUsePeriodicNotesSettings('yearly') && pluginManager.getPlugin('periodic-notes')?.settings?.yearly) ||
            {};
        return {
...
class DatePickerModal extends require$$0$1.Modal {
    ...
    onOpen() {
        ...
        const getDateStr = () => {
            ...
            let parsedDateString = parsedDate.moment.isValid() ? parsedDate.moment.format(momentFormat) : '';
            if (insertAsLink) {
...
(function (exports) {
    ...
    exports.TIME_UNITS_PATTERN = (0, pattern_1.repeatedTimeunitPattern)(
        `(?:(?:about|around)\\s{0,3})?`,
...
class ParsingComponents {
    ...
    getSystemTimezoneAdjustmentMinute(date) {
        ...
        const targetTimezoneOffset =
            (_b = (_a = this.get('timezoneOffset')) !== null && _a !== void 0 ? _a : this.reference.timezoneOffset) !==
                null && _b !== void 0
                ? _b
...
const PATTERN_WITH_PREFIX = new RegExp(
    `(?:within|in|for)\\s*` +
        `(?:(?:about|around|roughly|approximately|just)\\s*(?:~\\s*)?)?(${constants_1$C.TIME_UNITS_PATTERN})(?=\\W|$)`,
    'i',
);
...
const PATTERN$A = new RegExp(
    `(${(0, pattern_1$g.matchAnyPattern)(constants_1$A.MONTH_DICTIONARY)})` +
        '(?:-|/|\\s*,?\\s*)' +
...
class ENMonthNameParser extends AbstractParserWithWordBoundary_1$O.AbstractParserWithWordBoundaryChecking {
    ...
    innerExtract(context, match) {
        ...
        const result = context.createParsingResult(
            match.index + (match[PREFIX_GROUP$4] || '').length,
...
const PATTERN$v = new RegExp(
    `(${constants_1$w.TIME_UNITS_PATTERN})\\s{0,5}(?:later|after|from now|henceforth|forward|out)` + '(?=(?:\\W|$))',
    'i',
);
const STRICT_PATTERN$2 = new RegExp(
    '' + '(' + constants_1$w.TIME_UNITS_PATTERN + ')' + '(later|from now)' + '(?=(?:\\W|$))',
    'i',
);
    ...
    refine(context, results) {
        ...
        results.forEach((result) => {
            ...
            const extractedTimezoneOffset =
                (_b =
                    (_a = timezoneOverrides[timezoneAbbr]) !== null && _a !== void 0
                        ? _a
...
casualReferences.tonight =
    casualReferences.tomorrow =
    casualReferences.yesterday =
    casualReferences.today =
    casualReferences.now =
        void 0;
...
var __importStar$5 =
    ...
    function (mod) {
        ...
        if (mod != null)
            for (var k in mod)
                if (k !== 'default' && Object.prototype.hasOwnProperty.call(mod, k)) __createBinding$5(result, mod, k);
...
function toDayJSWeekday(refDate, offset, modifier, locale) {
    ...
    const weekStart =
        (_a = locale === null || locale === void 0 ? void 0 : locale.weekStart) !== null && _a !== void 0 ? _a : 0;
...
const PATTERN$q = new RegExp(
    `(this|next|last|past)\\s*(${(0, pattern_1$c.matchAnyPattern)(constants_1$u.TIME_UNIT_DICTIONARY)})(?=\\s*)` +
        '(?=\\W|$)',
    'i',
);
...
class ENTimeUnitCasualRelativeFormatParser
    extends AbstractParserWithWordBoundary_1$E.AbstractParserWithWordBoundaryChecking
{
    innerPattern() {
        return PATTERN$o;
    }
...
var __importStar$4 =
    ...
    function (mod) {
        ...
        if (mod != null)
            for (var k in mod)
                if (k !== 'default' && Object.prototype.hasOwnProperty.call(mod, k)) __createBinding$4(result, mod, k);
        __setModuleDefault$4(result, mod);
        return result;
    };
var __importDefault$u =
    (commonjsGlobal && commonjsGlobal.__importDefault) ||
    function (mod) {
        return mod && mod.__esModule ? mod : { default: mod };
    };
...
const SECOND_REG_PATTERN$2 = new RegExp(
    '^\\s*(\\-|\\–|\\~|\\〜|[àa]|\\?)\\s*' +
        '(\\d{1,2})(?:h|:)?' +
        '(?:(\\d{1,2})(?:m|:)?)?' +
        '(?:(\\d{1,2})(?:s|:)?)?' +
...
constants$3.parseYear =
    constants$3.YEAR_PATTERN =
...
class NLTimeUnitWithinFormatParser extends AbstractParserWithWordBoundary_1$m.AbstractParserWithWordBoundaryChecking {
    innerPattern() {
        return new RegExp(
            `(?:binnen|in|binnen de|voor)\\s*` + '(' + constants_1$i.TIME_UNITS_PATTERN + ')' + `(?=\\W|$)`,
            'i',
        );
    }
    innerExtract(context, match) {
...
const AbstractParserWithWordBoundary_1$j = AbstractParserWithWordBoundary;
const PATTERN$c = new RegExp(
    `(${(0, pattern_1$2.matchAnyPattern)(constants_1$f.MONTH_DICTIONARY)})` +
        `\\s*` +
        `(?:` +
...
const PATTERN$9 = new RegExp(
    `(deze|vorige|afgelopen|komende|over|\\+|-)\\s*(${constants_1$d.TIME_UNITS_PATTERN})(?=\\W|$)`,
    'i',
);
class NLTimeUnitCasualRelativeFormatParser
    extends AbstractParserWithWordBoundary_1$f.AbstractParserWithWordBoundaryChecking
{
    innerPattern() {
        return PATTERN$9;
    }
...
const AbstractParserWithWordBoundary_1$e = AbstractParserWithWordBoundary;
const pattern_1 = pattern;
const PATTERN$8 = new RegExp(
    `(dit|deze|komende|volgend|volgende|afgelopen|vorige)\\s*(${(0, pattern_1.matchAnyPattern)(constants_1$c.TIME_UNIT_DICTIONARY)})(?=\\s*)` +
        '(?=\\W|$)',
    'i',
);
...
const PATTERN$7 = new RegExp(
    '' + '(' + constants_1$b.TIME_UNITS_PATTERN + ')' + '(?:geleden|voor|eerder)(?=(?:\\W|$))',
    'i',
);
...
const AbstractParserWithWordBoundary_1$c = AbstractParserWithWordBoundary;
const PATTERN$6 = new RegExp(
    '' + '(' + constants_1$a.TIME_UNITS_PATTERN + ')' + '(later|na|vanaf nu|voortaan|vooruit|uit)' + '(?=(?:\\W|$))',
    'i',
);
const STRICT_PATTERN = new RegExp(
    '' + '(' + constants_1$a.TIME_UNITS_PATTERN + ')' + '(later|vanaf nu)' + '(?=(?:\\W|$))',
    'i',
);
...
class ZHHantDateParser extends AbstractParserWithWordBoundary_1$a.AbstractParserWithWordBoundaryChecking {
    ...
    innerExtract(context, match) {
        ...
        if (isNaN(month)) month = (0, constants_1$9.zhStringToNumber)(match[MONTH_GROUP$1]);
        result.start.assign('month', month);
        if (match[DAY_GROUP$1]) {
            let day = parseInt(match[DAY_GROUP$1]);
            if (isNaN(day)) day = (0, constants_1$9.zhStringToNumber)(match[DAY_GROUP$1]);
            result.start.assign('day', day);
        } else {
        ...
        if (match[YEAR_GROUP$1]) {
            ...
            if (isNaN(year)) year = (0, constants_1$9.zhStringToYear)(match[YEAR_GROUP$1]);
            result.start.assign('year', year);
        } else {
...
const PATTERN$4 = new RegExp(
    '(?<prefix>上|今|下|這|呢)(?:個)?(?:星期|禮拜|週)(?<weekday>' +
        Object.keys(constants_1$7.WEEKDAY_OFFSET).join('|') +
        ')',
);
...
const SECOND_REG_PATTERN$1 = new RegExp(
    '(?:^\\s*(?:到|至|\\-|\\–|\\~|\\〜)\\s*)' +
        '(?:' +
        '(今|明|前|大前|後|大後|聽|昨|尋|琴)(早|朝|晚)|' +
        '(上(?:午|晝)|朝(?:早)|早(?:上)|下(?:午|晝)|晏(?:晝)|晚(?:上)|夜(?:晚)?|中(?:午)|凌(?:晨))|' +
...
(function (exports) {
    ...
    function createConfiguration() {
        ...
        configuration.refiners = configuration.refiners.filter(
            (refiner) => !(refiner instanceof ExtractTimezoneOffsetRefiner_1.default),
        );
        return configuration;
    }
    exports.createConfiguration = createConfiguration;
...
class ZHHansDateParser extends AbstractParserWithWordBoundary_1$4.AbstractParserWithWordBoundaryChecking {
    ...
    innerExtract(context, match) {
        ...
        if (isNaN(month)) month = (0, constants_1$4.zhStringToNumber)(match[MONTH_GROUP]);
        result.start.assign('month', month);
        if (match[DAY_GROUP]) {
            let day = parseInt(match[DAY_GROUP]);
            if (isNaN(day)) day = (0, constants_1$4.zhStringToNumber)(match[DAY_GROUP]);
            result.start.assign('day', day);
        } else {
            result.start.imply('day', startMoment.date());
        }
        if (match[YEAR_GROUP]) {
            ...
            if (isNaN(year)) year = (0, constants_1$4.zhStringToYear)(match[YEAR_GROUP]);
            result.start.assign('year', year);
        } else {
            result.start.imply('year', startMoment.year());
        }
...
const PATTERN$1 = new RegExp(
    '(?<prefix>上|下|这)(?:个)?(?:星期|礼拜|周)(?<weekday>' + Object.keys(constants_1$2.WEEKDAY_OFFSET).join('|') + ')',
);
...
(function (exports) {
    ...
    var hans_1 = hans;
    Object.defineProperty(exports, 'hans', {
        enumerable: true,
        get: function () {
            return hans_1.hans;
        },
...
class NLDParser {
    ...
    getParsedDate(selectedText, weekStartPreference) {
        ...
        const weekdayIsCertain =
            (_a = initialParse[0]) === null || _a === void 0 ? void 0 : _a.start.isCertain('weekday');
...
class NLDSettingsTab extends require$$0$1.PluginSettingTab {
    ...
    display() {
        ...
        new require$$0$1.Setting(containerEl)
            ...
            .addToggle((toggle) =>
                toggle.setValue(this.plugin.settings.isAutosuggestEnabled).onChange((value) =>
                    __awaiter(this, void 0, void 0, function* () {
        ...
        new require$$0$1.Setting(containerEl)
            ...
            .addToggle((toggle) =>
                toggle.setValue(this.plugin.settings.autosuggestToggleLink).onChange((value) =>
                    __awaiter(this, void 0, void 0, function* () {
...
class DateSuggest extends require$$0$1.EditorSuggest {
    ...
    getDateSuggestions(context) {
        ...
        return [{ label: 'Today' }, { label: 'Yesterday' }, { label: 'Tomorrow' }].filter((items) =>
            items.label.toLowerCase().startsWith(context.query),
        );
    }
